...
class SwiftQueueManagerTests: XCTestCase {
    ...
    func testIsReachableCellular() {
        ...
        let queueJob = SwiftQueueJob(json: json, creator: [creator])
        XCTAssertTrue(queueJob?.checkIsReachable() ?? false)
    }

    func testIsReachableWifi() {
        let job = TestJob()
        let type = UUID().uuidString
